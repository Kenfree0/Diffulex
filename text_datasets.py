...
def helper_tokenize(sentence_lst, vocab_dict, seq_len):
    ...
    def tokenize_function(examples):
        ...
        result_dict = {'input_id_x': input_id_x, 'input_id_y': input_id_y, 'label': lll}
    ...
    def pad_function(group_lst):
        ...
        group_lst['label'] = _collate_batch_helper(group_lst['label'], 0, max_length)
        return group_lst
...
def get_corpus(data_args, seq_len, split='train', loaded_vocab=None):
    ...
    with open(path, 'r') as f_reader:
        for row in f_reader:
            ...
            sentence_lst['label'].append(json.loads(row)['label'])

    print('### Data samples...\n', sentence_lst['src'][:2], sentence_lst['trg'][:2], sentence_lst['label'][:2])
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        with torch.no_grad():
            ...
            out_kwargs['label'] = np.array(self.text_datasets['train'][idx]['label'])
